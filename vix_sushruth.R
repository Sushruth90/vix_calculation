...
F1 <- min_diff_near + exp(rf_rate * T_exp_near) * (mid_call_near - mid_put_near)
F2 <- min_diff_next + exp(rf_rate * T_exp_next) * (mid_call_next - mid_put_next)
...
filter_otm_puts <- function(puts, K0) {
  puts %>%
    filter(STRIKEPRICE < K0, BID_PRICE > 0) %>%
    arrange(desc(STRIKEPRICE))
}

# Select out-of-the-money puts below K0 for near-term
near_term_otm_puts <- filter_otm_puts(near_term_puts, K0_near)

# Select out-of-the-money puts below K0 for next-term
next_term_otm_puts <- filter_otm_puts(next_term_puts, K0_next)

# Print the results
print("Out-of-the-money puts (Near-term) below K0:")
print(near_term_otm_puts)

print("Out-of-the-money puts (Next-term) below K0:")
